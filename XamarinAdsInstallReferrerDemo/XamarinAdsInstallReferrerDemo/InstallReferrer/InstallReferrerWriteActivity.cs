...
namespace XamarinAdsInstallReferrerDemo.InstallReferrer
{
    ...
    public class InstallReferrerWriteActivity : BaseActivity,View.IOnClickListener
    {
        ...
        protected void Init()
        {
            ...
            mPackageNameEt = FindViewById<EditText>(Resource.Id.package_name_et);
            // Create the "install_referrer" EditText. User can write install referrer info.
            mInstallReferrerEt = FindViewById<EditText>(Resource.Id.install_referrer_et);
            // Create the "delete" button, which tries to delete existed install referrer according package name.
            mDeleteBtn = FindViewById<Button>(Resource.Id.delete_btn);
            mDeleteBtn.SetOnClickListener(this);
            // Create the "save" button, which tries to save what just typed.
            mSaveBtn = FindViewById<Button>(Resource.Id.save_btn);
        ...
        private void SaveInstallReferrer()
        {
            ...
            string pkgName = mPackageNameEt.Text.ToString();
            string installReferrer = mInstallReferrerEt.Text.ToString();
        ...
        private void SaveOrDelete(string pkgName, string installReferrer, bool isDelete)
        {
            ...
            ISharedPreferences sharedPreferences = GetSharedPreferences(Constants.INSTALL_REFERRER_FILE, FileCreationMode.Private);
